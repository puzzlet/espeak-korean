  ...
  [['config', config], ['phontab', phontab], ['phonindex', phonindex], ['phondata', phondata], ['intonations', intonations], ['en_dict', en_dict] , ['ko_dict', ko_dict] ].forEach(function(pair) {
    var id = pair[0];
    var data = pair[1];
  ...
  FS.createDataFile('/espeak/espeak-data/voices', 'ko', ko, true, false);
  ...
  Module.arguments = [
    ...
    '-v', args['voice'] ? String(args['voice']) : 'ko',
    // end options
    '--path=/espeak',
    this['text']
  ];
